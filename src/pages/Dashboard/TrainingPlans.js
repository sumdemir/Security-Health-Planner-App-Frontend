              ...
              {trainingLists.map((trainingPlan) => (
                <Col key={trainingPlan.id} flex="0 0 auto">
                    ...
                    title={
                      <div style={{ textAlign: 'center', whiteSpace: 'normal' }}>
                        ...
                        <div style={{ fontSize: '12px', color: 'gray' }}>
                          {new Date(trainingPlan.createdAt).toLocaleDateString()} -{' '}
                          {new Date(trainingPlan.createdAt).toLocaleTimeString()}
                        </div>
                      </div>
                    }
                    bordered={true}
                  ...
                  >
                    <div
                      style={{
                        maxHeight: trainingLists.length === 1 ? 'none' : '150px', // Tek öğe varsa sınırsız yükseklik
                        overflowY: trainingLists.length === 1 ? 'visible' : 'auto', // Tek öğe varsa scroll bar olmasın
                        paddingRight: trainingLists.length === 1 ? '0' : '10px',
                      }}
                    >
                      <p
                        style={{
